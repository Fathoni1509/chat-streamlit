...
import streamlit as st
import pika, threading, json, os
from datetime import datetime

# Ambil URL dari Railway env
RABBIT_URL = os.getenv("AMQP_URL")  
QUEUE_NAME = "chat_queue"

# Input username sekali saja
if "username" not in st.session_state:
    st.session_state["username"] = st.text_input("Masukkan nama Anda:")
...
if "consumer_thread" not in st.session_state and RABBIT_URL:
    t = threading.Thread(target=start_consumer, daemon=True)
    t.start()
    st.session_state["consumer_thread"] = t
...
# Input pesan
msg = st.text_input("Ketik pesan:")
if st.button("Kirim") and msg and st.session_state["username"]:
    params = pika.URLParameters(RABBIT_URL)
    connection = pika.BlockingConnection(params)
    channel = connection.channel()
    channel.queue_declare(queue=QUEUE_NAME, durable=True)
